...
class SmartMeasurer extends StatefulWidget {
  ...
  /// If true, the widget returned by [builder] is prevented from ever
  /// painting outside the space available from the parent: it is clipped to
  /// that space (or, if [shrinkToFit] is also true, scaled down to fit
  /// instead of being clipped). Has no effect on the axis along which
  /// [scrollable] is true, since scrolling already handles content larger
  /// than the available space on that axis — combine the two to also cap
  /// the cross-axis size.
  ///
  /// Note this only prevents *visual* overflow (painting/clipping); it does
  /// not shrink the content unless [shrinkToFit] is also set.
  final bool constrainToAvailableSpace;

  /// Only relevant when [constrainToAvailableSpace] is true and content
  /// does not fit in the available space on the non-scrollable axis. If
  /// true, that content is scaled down (preserving aspect ratio, like
  /// [BoxFit.scaleDown]) to fit instead of being clipped. Defaults to
  /// false, which clips instead.
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        ...
        if (widget.scrollable) {
          ...
          // A ConstrainedBox alone changes nothing here: `result` already
          // receives `constraints` as its incoming BoxConstraints from the
          // ambient LayoutBuilder, so re-applying the same max is a no-op.
          // The actual fix is ClipRect: Flex-based children (Row/Column)
          // don't clip themselves when they overflow their own bounds, they
          // just paint a debug overflow indicator past them — ClipRect is
          // what actually keeps `result` visually inside the space it was
          // given, matching this flag's contract.
          result = ClipRect(
